...
void loop() {
  ...
  if ((millis() - lastMilli) >= LOOPTIME) { // вход в тайм-луп
    unsigned long current_time = millis();
     ...
     if (abs(pulses_1) < 5){                                                   //Avoid taking in account small disturbances
      speed_act_left = 0;
    }
    else {
      speed_act_left=((pulses_1 *PI* wheelDiam)/pulsesperturn)*(1000/LOOPTIME);           // calculate speed of left wheel
      encoder1_msg.data = speed_act_left;
      encoder1_pub.publish(&encoder1_msg);
      pulses_1 = 0;
    }

    if (current_time - last_time_2 >= 1000) { // Обновляем данные каждую секунду
      float rpm_2 = (60.0 * 1000.0 / pulsesperturn) / (current_time - last_time_2) * pulses_2;
      speed_act_right = rpm_2 * 3.1416 * wheelDiam / 60.0; // Вычисляем скорость в [м/с]
    ...
    encoder2_msg.data = PWM_rightMotor;
